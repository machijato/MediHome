...
const mapCategoryIdToType = (categoryId: ProviderListing['category_id']): Provider['type'] => {
  if (categoryId === 'physio' || categoryId === 1 || categoryId === '1') return 'physio';
  if (categoryId === 'nurse' || categoryId === 2 || categoryId === '2') return 'nurse';
  if (categoryId === 'equipment' || categoryId === 3 || categoryId === '3') return 'equipment';
  if (categoryId === 'transport' || categoryId === 4 || categoryId === '4') return 'transport';
  return 'other';
};
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      if (error) {
        ...
        const mappedProviders: Provider[] = (data as ProviderListing[]).map((listing) => ({
          id: String(listing.id),
          name: listing.title,
          type: mapCategoryIdToType(listing.category_id),
          rating: 0,
          reviewsCount: 0,
          price: listing.price_from ? `od ${listing.price_from}€` : 'Cijena na upit',
          location: listing.city,
          image: 'https://picsum.photos/seed/provider-listing/400/300',
