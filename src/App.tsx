...
function ListingDetailPage() {
  ...
  if (isLoading) {
    return (
      <main data-testid="listing-detail-page" className="max-w-6xl mx-auto px-4 py-12">
    ...
    return (
      <main data-testid="listing-detail-page" className="max-w-4xl mx-auto px-4 py-12">
        <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 text-center">
          <h1 className="text-2xl font-bold text-slate-900 mb-3">Oglas nije pronađen</h1>
          <p className="text-slate-600">Traženi oglas možda više nije dostupan ili je poveznica netočna.</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main data-testid="listing-detail-page" className="max-w-6xl mx-auto px-4 py-12">
      <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8 mb-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            <h1 data-testid="listing-detail-title" className="text-3xl font-bold text-slate-900">{listing.title}</h1>
            <div className="flex flex-wrap gap-3 text-sm">
              <span data-testid="listing-detail-city" className="inline-flex items-center px-3 py-1 rounded-full bg-slate-100 text-slate-700">
                {listing.city || 'Nepoznato'}
              </span>
          ...
          <p data-testid="listing-detail-price" className="inline-flex items-center px-4 py-2 rounded-xl bg-slate-900 text-white text-lg font-bold">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <article className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="h-80 md:h-96 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-500">
            Slika oglasa uskoro dostupna
          </div>
        </article>

        <aside className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 h-fit space-y-5">
          <h2 className="text-xl font-bold text-slate-900">Osnovne informacije</h2>
          <div className="space-y-3 text-slate-700">
            <div>
              <p className="text-sm text-slate-500">Cijena</p>
              <p className="font-semibold">{[listing.price_from, listing.price_unit].filter(Boolean).join(' ') || 'Po dogovoru'}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">Lokacija</p>
              <p>{listing.city || 'Nepoznato'}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">Kategorija</p>
              <p>{listing.service_categories?.name || 'Nepoznato'}</p>
            </div>
          </div>

          <div className="pt-4 border-t border-slate-200">
            <h3 className="text-lg font-bold text-slate-900 mb-2">Pružatelj usluge</h3>
            <p data-testid="listing-detail-provider" className="text-slate-700 font-semibold">
              {listing.provider_profiles?.display_name || 'Nepoznato'}
            </p>
            <p className="text-sm text-slate-500 mt-1">{listing.provider_profiles?.provider_type || 'Pružatelj usluge'}</p>
          </div>
        </aside>
      </section>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8">
        ...
        <p data-testid="listing-detail-description" className="text-slate-700 leading-relaxed whitespace-pre-line">
          {listing.description || 'Opis nije dostupan.'}
        </p>
      </section>
    </main>
  );
